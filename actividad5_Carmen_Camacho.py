...
def procesar_datos(symbols):
    ...
    # Se filtran los valores None y se combinan los DataFrames
    resultados = [df for df in resultados if df is not None]
    if resultados:
        df_principal = pd.concat(resultados, axis=1)
        # Se verifica si "date" ya existe
        if "date" not in df_principal.columns:
            df_principal.insert(0, "date", resultados[0]["date"])  # Se asegura que "date" sea la primera columna
