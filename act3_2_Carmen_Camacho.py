...
# paralelas en un thread.

# Para esta actividad:
# Toda la parte de obtención de datos históricos de los símbolos se ha movido a un solo hilo usando Thread.
# Se utiliza un hilo para ejecutar la descarga de los datos de todos los símbolos al mismo tiempo,
# pero dentro de un solo hilo de ejecución principal.
...
def procesar_datos(symbols):
    ...
    def worker():
        ...
        for symbol in symbols:
            resultados.append(obtener_datos_simbolo(symbol))

    hilo = Thread(target=worker)
    hilo.start()
    hilo.join()
